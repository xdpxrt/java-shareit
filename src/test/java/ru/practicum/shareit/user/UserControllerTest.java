...
public class UserControllerTest {
    ...
    @Test
    void addUserEmailNotValidTest() throws Exception {
        UserDTO badUser = new UserDTO(1, "user", "user.com");
    ...
    @Test
    void getUserNotValidTest() throws Exception {
        when(userService.getUser(anyLong())).thenReturn(userDTO);
        mvc.perform(get("/users/{id}", -1)
                ...
                .getResponse()
                .getContentAsString();
        verify(userService, never()).getUser(1);
    }
    ...
    @Test
    void updateUserEmailNotValidTest() throws Exception {
        UserDTO badUser = new UserDTO(1, "user", "user.com");
        mvc.perform(patch("/users/{id}", -1)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(badUser)))
                ...
                .getResponse()
                .getContentAsString();
        verify(userService, never()).updateUser(badUser);
    }

    @Test
    void updateUserNameNotValidTest() throws Exception {
        UserDTO badUser = new UserDTO(1, "", "user@user.com");
        mvc.perform(patch("/users/{id}", -1)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(badUser)))
    ...
    @Test
    void deleteUserNotValidTest() throws Exception {
        mvc.perform(delete("/users/{id}", -1)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andReturn()
